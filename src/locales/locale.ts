...
function getLocale() {
  const queryLang = new URL(location.href).searchParams.get(localeKey)
  const localLang =
...
function setLocaleContext(
  context: Context<{ locale: string; reloadAppLocale: Function }>
) {
  localeContext = context
}

export { setLocale, getLocale, setLocaleContext }
